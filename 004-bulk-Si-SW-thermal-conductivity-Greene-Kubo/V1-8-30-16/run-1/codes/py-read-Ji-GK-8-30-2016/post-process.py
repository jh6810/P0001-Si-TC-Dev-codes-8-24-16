...
Jx=np.array(Jx)-np.mean(Jx); 
Jy=np.array(Jy)-np.mean(Jy); 
Jz=np.array(Jz)-np.mean(Jz); 

#Jy=np.array(Jx)-np.mean(Jx); 
#Jz=np.array(Jx)-np.mean(Jx);
...
while(k<Np): 
	...
	Jxsub=np.array(Jx[k:k+di])
	Jysub=np.array(Jy[k:k+di])
	Jzsub=np.array(Jz[k:k+di])

	if(len(tsub)==di and len(Jxsub)==di):
		print "len(tsub),len(Jxsub)",len(tsub),len(Jxsub)
		...
		Jx_SPD_i=(np.absolute(fft(Jxsub)/N)**2.0)
		Jy_SPD_i=(np.absolute(fft(Jysub)/N)**2.0)
		Jz_SPD_i=(np.absolute(fft(Jzsub)/N)**2.0)

		J_SPD_i=(Jx_SPD_i+Jy_SPD_i+Jz_SPD_i)/3.0

		KE_SPD_i=(np.absolute(fft(KEsub)/N)**2.0)

		Jx_ACF_i=np.real(N*ifft(Jx_SPD_i))
		Jy_ACF_i=np.real(N*ifft(Jy_SPD_i))
		Jz_ACF_i=np.real(N*ifft(Jz_SPD_i))

		KE_ACF_i=np.real(N*ifft(KE_SPD_i))

		J_ACF_i=(Jx_ACF_i+Jy_ACF_i+Jz_ACF_i)/3.0
